...
var Fragment = 7;
var Mode = 8;
var ContextConsumer = 9; // 消费
var ContextProvider = 10; // 生产
var ForwardRef = 11;
var Profiler = 12;
...
/**
 * @desc 把 SchedulerPriority 转换成 ReactPriorityLevel
*/
...
/**
 * @desc 把 ReactPriorityLevel 转换成 SchedulerPriority
*/
...
/**
 * @desc 任务回调函数，react的任务调度是一个异步的过程
*/
...
function insertInContainerBefore(container, child, beforeChild) {
  if (container.nodeType === COMMENT_NODE) {
    ...
    container.insertBefore(child, beforeChild); // 原生DOM操作
  }
}
...
/**
 * @desc 返回updateQueue对象
*/
function initializeUpdateQueue(fiber) {
  // 参考 1.u1 关闭黑夜模式 2. 受控的用户输入 demo
  // https://react.iamkasong.com/state/priority.html#%E4%BE%8B%E5%AD%90
  var queue = {
    baseState: fiber.memoizedState, // 本次更新前该Fiber节点的state
    firstBaseUpdate: null, // 本次更新前该Fiber节点已保存的Update。以链表形式存在，链表头为firstBaseUpdate，链表尾为lastBaseUpdate
    lastBaseUpdate: null, // 之所以在更新产生前该Fiber节点内就存在Update，是由于某些Update优先级较低所以在上次render阶段由Update计算state时被跳过。
    ...
    effects: null // 数组。保存update.callback !== null的Update（也就是setState时有回调需要处理的Update），在commit之后, 会依次调用这里的回调函数.
  };
...
function createUpdate(eventTime, lane) {
  // Update对象
  var update = { 
    eventTime: eventTime, // 发起update事件的时间(17.0.1中作为临时字段, 即将移出)
    ...
    callback: null, // setState的第二个参数 完成更新后的回调函数
    next: null // 与其他Update连接形成链表(updateQueue)
  };
  return update;
}
...
function processUpdateQueue(workInProgress, props, instance, renderLanes) {
  ...
  if (firstBaseUpdate !== null) {
    ...
    // 遍历pendingQueue (遍历pendingQueue === null时退出循环)
    do {
    ...
    workInProgress.memoizedState = newState; // setState后计算出新的state
  }
...
function ChildReconciler(shouldTrackSideEffects) {
  function deleteChild(returnFiber, childToDelete) {
    ...
    if (deletions === null) {
      returnFiber.deletions = [childToDelete]; // 存储将要被删除的子节点(有啥用)
      returnFiber.flags |= ChildDeletion; // 标记位置,替代react16中的effectTags，给当前节点的
    } else {
  ...
  function deleteRemainingChildren(returnFiber, currentFirstChild) {
    ...
    while (childToDelete !== null) {
      ...
      childToDelete = childToDelete.sibling; // 兄弟节点
    }
  ...
  /**
   * @desc 单节点diff
   * @params returnFiber: workInProgress，也就是下一次渲染要展示的树
   * @params currentFirstChild: current树中当前节点的第一个子节点
  */
  function reconcileSingleElement(returnFiber, currentFirstChild, element, lanes) {
    ...
    while (child !== null) {
      ...
      if (child.key === key) {
        var elementType = element.type;
        // 假设是 React.Fragment类型(类似<></>)，表示是一个空节点，特殊处理
        if (elementType === REACT_FRAGMENT_TYPE) {
          if (child.tag === Fragment) {
            ...
            // 这里表示返回复用的fiber(可以看到 existing是 current树中的节点，不需要再次创建)
            return existing;
          }
        } else {
  ...
  function reconcileChildFibers(returnFiber, currentFirstChild, newChild, lanes) {
    ...
    // object、number、string类型 代表同级只有一个节点 (单节点diff)
    if (isObject) {
      switch (newChild.$$typeof) {
        case REACT_ELEMENT_TYPE:
    ...
    // 当newArray类型为Array,同级有多个节点 (多节点diff)
...
function beginWork(current, workInProgress, renderLanes) {
  ...
  if (current !== null) {
    ...
    var oldProps = current.memoizedProps; // 上一次生成子节点时用到的属性, 生成子节点之后保持在内存中
    var newProps = workInProgress.pendingProps; // 从`ReactElement`对象传入的 props. 用于和`fiber.memoizedProps`比较可以得出属性是否变动
    ...
    didReceiveUpdate = false; // 该字段表示 可以直接复用前一次更新的子Fiber，不需要新建子Fiber
...
var updateHostComponent$1; // beginWork时调用 updateHostComponent; completeWork时调用 updateComponent$1
  ...
  if (!didBailout) {
    ...
    if ( (completedWork.mode & ProfileMode) !== NoMode) {
      ...
      while (child !== null) {
        ...
        subtreeFlags |= child.subtreeFlags; // 我感觉react17中设置 subtreeFlags(effectList) 就是在这里收集的？
    ...
    case HostComponent:
      {
        ...
        if (current !== null && workInProgress.stateNode != null) {
          ...
          // mount阶段，调用appendAllChildren来构建离屏DOM树
          if (!newProps) {
          ...
          if (_wasHydrated) {
            ...
            // mount 时，仅有rootFiber存在Placement effectTag（v17中叫做 flags）
            // 由于completeWork属于“归”阶段调用的函数，每次调用appendAllChildren时都会将已生成的子孙DOM节点插入当前生成的DOM节点下。
            // 那么当“归”到rootFiber时，我们已经有一个构建好的离屏DOM树。就能在commit阶段一次通过一次插入DOM操作将整棵DOM树插入页面
...
// 在这里调用 getSnapshotBeforeUpdate | useEffect 生命周期函数
...
/**
 * @desc 该方法会遍历effectList，执行所有useLayoutEffect hook的销毁函数
 * @example useEffect(() => {
...
/**
 * @desc 调用生命周期函数和hook相关操作
 * 触发状态更新的this.setState如果赋值了第二个参数回调函数，也会在此时调用？？？ setState的回调函数存在哪里? 存在Update对象的callback中
*/
      ...
      case ClassComponent:
        {
          ...
          if (finishedWork.flags & Update) {
            if (current === null) {
              ...
              {
                // commit阶段中的layout阶段，会根据current === null 来决定调用componentDidMount/Update
...
function commitAttachRef(finishedWork) {
  ...
  if (ref !== null) {
    ...
    if (typeof ref === 'function') {
      { 
        // ref会在这里执行,可以看到，这里实际上是对真实DOM的引用....
        ref(instanceToUse);
      }
    } else {
      {
...
/**
 * 1. 递归调用Fiber节点及其子孙Fiber节点中fiber.tag为ClassComponent的componentWillUnmount (opens new window)生命周期钩子
 *    从页面移除Fiber节点对应DOM节点
 * 2. 解绑ref
 * 3. 调度useEffect的销毁函数
*/
...
/**
 * update effect意味着Fiber节点需要更新。 调用的方法为commitwork
*/
function commitWork(current, finishedWork) {
  ...
  switch (finishedWork.tag) {
    ...
    case MemoComponent:
    case SimpleMemoComponent:
    // 注意这里的语法 FunctionComponent、ForwardRef、MemoComponent、SimpleMemoComponent都会走这里的
      {
...
/** commit阶段中的 mutation阶段
 * 1. 根据effectTag调用不同的处理函数处理Fiber
*/
...
function commitMutationEffects_begin(root, renderPriorityLevel) {
  // 遍历 effectList 
  // Dan Abramov：effectList相较于Fiber树，就像圣诞树上挂的那一串彩灯。
  while (nextEffect !== null) {
    var fiber = nextEffect; // TODO: Should wrap this in flags check, too, as optimization
    ...
    case Update:
      {
        ...
        // Update effectTag的时候，意味着该Fiber节点需要更新。
...
function workLoopSync() {
  ...
  while (workInProgress !== null) {
    console.log('workLoopSync func param : workInProgress', workInProgress);
...
function performUnitOfWork(unitOfWork) {
  ...
  console.log('performUnitOfWork func apply');
...
/**
 * 为了在commit阶段不再遍历一次Fiber树寻找effectTag !== null的Fiber节点，产生额外的遍历消耗。
 * 在completeUnitOfWork中，每个执行完completeWork且存在effectTag的Fiber节点
 * 会被保存在一条被称为effectList(v16 v17中改成 subtreeFlags，但是貌似需要手动开启才能看到效果，这里看不到)的单向链表中,
*/
  ...
  do {
    ...
    if ((completedWork.flags & Incomplete) === NoFlags) {
      ...
      if ( (completedWork.mode & ProfileMode) !== NoMode) {
        ...
        while (child !== null) {
          actualDuration += child.actualDuration;
          child = child.sibling; // 王朝的故事，深度优先遍历
        }
...
/**
 * 1. 在rootFiber.firstEffect上保存了一条需要执行副作用的Fiber节点的单向effctList,这些Fiber节点中的updateQueue中保存了变化的props
 * 这些副作用需要在commit阶段执行
 * 2. 除此之外，一些生命周期函数，比如(componentDidXXX)、hook(比如useEffect)需要在commit阶段执行
 * 3. commit阶段只要分成3个部分(1. before mutation 2. mutation 3.layout)
*/
...
function commitRootImpl(root, renderPriorityLevel) {
  // commit阶段 step1 before mutation --------------------------------------- 
  do {
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    // 重点关注下这行代码!!! workInProgress树和current树的切换
...
/**
 * @desc 构建workInProgress树
 * @params pendingProps 新的props
*/
function createWorkInProgress(current, pendingProps) {
  var workInProgress = current.alternate; // 获取current树对应的workInProgress树的Fiber
  ...
  if (workInProgress === null) {
    ...
    workInProgress.alternate = current; // 第一次diff,需要将workInProgress.alternate 指向 current
  ...
  workInProgress.flags = current.flags & StaticMask; // 这个地方的flags是diff的时候就确定的吗？？？ 如果是的话 感觉不太对，可以复用的时候难道不该打个Update标签吗
...
function createHostRootFiber(tag, strictModeLevelOverride) {
  var mode; // ConcurrentRoot | BlockingRoot | LegacyRoot
  ...
  // fiber树中所节点的mode都会和HostRootFiber.mode一致
  // (新建的 fiber 节点, 其 mode 来源于父节点),所以HostRootFiber.mode非常重要, 它决定了以后整个 fiber 树构建过程.
 ...
 * fiberRootNode有且仅有一个，但是rootFiber可以有多个(rootFiber是ReactDOM.render的返回结果)。
*/
...
function createFiberRoot(containerInfo, tag, hydrate, hydrationCallbacks, strictModeLevelOverride) {
  ...
  // 初始化updateQueue
  // updateQueue队列记录了该 fiber 节点的更新状态, 是实现组件更新的关键属性.
...
// 该对象暴露有render,unmount方法, 通过调用该实例的render方法, 可以引导 react 应用的启动
...
function legacyRenderSubtreeIntoContainer(parentComponent, children, container, forceHydrate, callback) {
  ...
  console.log('fiber -----', fiberRoot.current); // zr-log
